...
ExportSimulation::ExportSimulation(Ped::Model &model_, int maxSteps, std::string outputFilename_)
...
void ExportSimulation::serialize() {
	...
	} else if (impl == Ped::OMP_MV_HM_BN) {
		const struct dscu_agents_s *agents = model.get_dscu_agents_s();
		size_t num_agents = agents->size;
		file.write(reinterpret_cast<const char *>(&num_agents), sizeof(num_agents));
