...
IMPLS=("serial" "omp" "pthread" "simd" "cuda")
SCENARIOS=("hugeScenario.xml" "scenario.xml" "scenario_box.xml")
	...
	for max_steps in "${MAX_STEPS[@]}"; do
		...
		for ((i = 1; i <= RUNS; i++)); do
			ret=$(OMP_SCHEDULE="dynamic,1" OMP_NUM_THREADS=16 \
				run_demo 16 --omp --timing --max-steps="$max_steps" "$SCENARIO")
...
for impl in "${IMPLS[@]}"; do
	echo ""
	echo "############### $impl ###############"
	echo "Running $impl benchmarks..."

	for scenario in "${SCENARIOS[@]}"; do
		"run_$impl" "$scenario"
	done

	echo "Running $impl benchmarks complete"
done
