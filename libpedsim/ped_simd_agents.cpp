...
static inline __m512d fetch_dsts(ssize_t *destination_idx, double **coord,
								 const size_t agent_idx) {
	...
	return _mm512_set_pd(dsts[7], dsts[6], dsts[5], dsts[4], dsts[3], dsts[2],
						 dsts[1], dsts[0]);
}
...
void simd_computeNextDesiredPosition(const struct agents *agents,
	...
	__m512d dst_x =
		fetch_dsts(agents->destination_idx, agents->waypoints.x, agent_idx);
	__m512d diffX = _mm512_sub_pd(dst_x, agent_xd);

	__m512d dst_y =
		fetch_dsts(agents->destination_idx, agents->waypoints.y, agent_idx);
	__m512d diffY = _mm512_sub_pd(dst_y, agent_yd);

	__m512d diffX_sqr = _mm512_mul_pd(diffX, diffX);
	__m512d diffY_sqr = _mm512_mul_pd(diffY, diffY);

	__m512d len = _mm512_sqrt_pd(_mm512_add_pd(diffX_sqr, diffY_sqr));
	const __m512d dst_r =
		fetch_dsts(agents->destination_idx, agents->waypoints.y, agent_idx);
	...
	const __m512i nextDestination_idx_if =
		_mm512_mask_blend_epi64(overflow, wps_idx, minus_one);
	// Case 2: agent has not yet reached destination, continue to move towards
	// current destination
	const __m512i nextDestination_idx_else =
		_mm512_load_epi64(&agents->destination_idx[agent_idx]);

	const __m512i nextDestination_idx = _mm512_mask_blend_epi64(
		_kand_mask8(agentReachedDestination, non_empty_sz),
		nextDestination_idx_else, nextDestination_idx_if);

	// final nextDestination_idx
	_mm512_store_epi64(&agents->destination_idx[agent_idx],
					   nextDestination_idx);

	// no destination, no need to compute where to move to
	const __m512i dst_idx =
		_mm512_load_epi64(&agents->destination_idx[agent_idx]);
	// copy from src when bit is NOT set
	const __mmask8 has_dst = _mm512_cmpneq_epi64_mask(dst_idx, minus_one);

	const __m256i desiredPositionX =
		_mm256_load_epi32(&agents->desiredPositionX[agent_idx]);
	const __m256i desiredPositionY =
		_mm256_load_epi32(&agents->desiredPositionY[agent_idx]);

	dst_x = fetch_dsts(agents->destination_idx, agents->waypoints.x, agent_idx);
	diffX = _mm512_sub_pd(dst_x, agent_xd);
	dst_y = fetch_dsts(agents->destination_idx, agents->waypoints.y, agent_idx);
	diffY = _mm512_sub_pd(dst_y, agent_yd);
	diffX_sqr = _mm512_mul_pd(diffX, diffX);
	diffY_sqr = _mm512_mul_pd(diffY, diffY);
	len = _mm512_sqrt_pd(_mm512_add_pd(diffX_sqr, diffY_sqr));

	__m512d a =_mm512_add_pd(agent_xd, _mm512_div_pd(diffX, len));
	__m512d b = _mm512_add_pd(agent_xd, _mm512_div_pd(diffY, len));
	const __m256i desiredPositionX_new = _mm512_mask_cvt_roundpd_epi32(desiredPositionX, has_dst, a, _MM_FROUND_TO_NEAREST_INT |_MM_FROUND_NO_EXC);
	const __m256i desiredPositionY_new = _mm512_mask_cvt_roundpd_epi32(desiredPositionY, has_dst, b, _MM_FROUND_TO_NEAREST_INT |_MM_FROUND_NO_EXC);

	_mm256_store_epi32(&agents->desiredPositionX[agent_idx], desiredPositionX_new);
	_mm256_store_epi32(&agents->desiredPositionY[agent_idx], desiredPositionY_new);
}
