...
void mv_parallel_struct_regions_dinit(void) { free(agents_buckets); }

int mv_parallel_struct_setup_regions(std::vector<struct region_bn_s> &regions, const int agents_size) {
	static const size_t IDEAL_LOAD = agents_size / MAX_NUM_REGIONS;
...
void mv_parallel_struct_get_agents_in_region(struct dscu_agents_s *agents, struct region_bn_s *region) {
	...
	for (int i = 0; i < agents->size; i++) {
		const int x = agents->x[i];
		const int y = agents->y[i];
		if ((x >= region->x_start && x <= region->x_end) || (region->x_start == 0 && x < 0) ||
			(region->x_end == GRID_WIDTH && x > GRID_WIDTH)) {
			region->region_agents.push_back(i);
			struct pair_bn_s pair = {.x = x, .y = y};
...
static bool find_pair(std::vector<struct pair_bn_s> taken_positions, struct pair_bn_s pair) {
...
static bool
try_place_on_border(struct region_bn_s *region, struct dscu_agents_s *agents, int global_idx, int x, int y) {
	...
	const int prev_x = agents->x[global_idx];
	const int prev_y = agents->y[global_idx];
	if (prev_x == region->x_start || prev_x == region->x_end) {
		(*border)[prev_y].store(false, std::memory_order_release);
	}
	agents->x[global_idx] = x;
	agents->y[global_idx] = y;
...
static bool try_migrate_outside_grid(
	struct region_bn_s *region, struct dscu_agents_s *agents, int global_idx, int x, int y) {
...
static bool try_migrate(struct region_bn_s *region, struct dscu_agents_s *agents, int global_idx, int x, int y) {
	if (x < 0 || x > GRID_WIDTH) {
		return try_migrate_outside_grid(region, agents, global_idx, x, y);
	}
	struct region_bn_s *adjacent_region;
	std::vector<std::atomic<bool>> *border;
	std::vector<std::atomic<bool>> *prev_border;
...
static void
leave_border(struct region_bn_s *region, struct dscu_agents_s *agents, int global_idx, int x, int y) {
	std::vector<std::atomic<bool>> *prev_border;
	if (agents->x[global_idx] == region->x_start) {
		prev_border = &region->lborder;
	} else { // if (agents->x[global_idx] == region->x_end)
		prev_border = &region->rborder;
	}
...
void move_parallel_struct(struct region_bn_s *region, struct dscu_agents_s *agents, int agent_idx) {
	...
	const int getX = agents->x[global_idx];
	const int getY = agents->y[global_idx];

	int diffX = pDesired.x - getX;
	int diffY = pDesired.y - getY;
	struct pair_bn_s p1, p2;
	if (diffX == 0 || diffY == 0) {
	...
	for (size_t i = 0; i < NUM_ALTERNATIVES; i++) {
		...
		if (desired_x == region->x_start || desired_x == region->x_end) {
			success = try_place_on_border(region, agents, global_idx, desired_x, desired_y);
		} else if ((desired_x == region->x_start - 1 && getX == region->x_start) ||
				   (desired_x == region->x_end + 1 && getX == region->x_end)) {
			success = try_migrate(region, agents, global_idx, desired_x, desired_y);
		} else if (!find_pair(region->taken_positions, prioritizedAlternatives[i])) {
			// If the current position is not yet taken by any neighbor
			// Set the agent's position
			if (getX == region->x_start || getX == region->x_end) {
